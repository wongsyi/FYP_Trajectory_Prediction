...
from model.RNN_CGNN_2E_Att_Parallel import RNN_CGNN_2E_Att_Parallel

# Make newly created directories readable, writable and descendible for everyone (chmod 777)
os.umask(0)

root_path = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, root_path)  #first file to run when called in Terminal

log_dir = os.path.dirname(os.path.abspath(__file__))
logging.getLogger("pytorch_lightning").setLevel(logging.INFO)

parser = argparse.ArgumentParser()
parser = RNN_CGNN_2E_Att_Parallel.init_args(parser)
...
def main():
    ...
    model = RNN_CGNN_2E_Att_Parallel(args)
