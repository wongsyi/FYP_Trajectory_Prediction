...
class ArgoDataExtractor:
    ...
    def extract_data(self, filename):
        ...
        res_trajs = []
        for key in keys:
            idcs = objs[key] # Retrive the row of the vehicle in the csv
            tt = trajs[idcs] # Trajectory(x,y)
            ts = steps[idcs] # Steps visible in the frame
            rt = np.zeros((50, 3))
        ...
        origin = res_trajs[0, 19, :2].copy() # retrive the position where agent at t = 0 (20th frame)

        rotation = np.eye(2, dtype=np.float32)
        theta = 0
        if self.align_image_with_target_x: #Calculate rotation matrix like in robotics
            pre = res_trajs[0, 19, :2] - res_trajs[0, 18, :2] # Compute the vector wrp to AV
            theta = np.arctan2(pre[1], pre[0])
            rotation = np.asarray([[np.cos(theta), -np.sin(theta)],
                             [np.sin(theta), np.cos(theta)]], np.float32)

        res_trajs[:, :, :2] = np.dot(res_trajs[:, :, :2] - origin, rotation) #aligned
