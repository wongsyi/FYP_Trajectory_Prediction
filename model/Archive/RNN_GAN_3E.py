...
class RNN_GAN_3E(pl.LightningModule):
    def __init__(self, args):
        super(RNN_GAN_3E, self).__init__()
        self.args = args
    ...
    def forward(self, batch):
        ...
        out_agent_gnn_filtered = out_agent_gnn[main_vehicle_index]

        ### Decoder ###
        out_linear = self.decoder_residual(out_agent_gnn_filtered, self.is_frozen)
...
class AgentGnn(nn.Module):
    def __init__(self, args):
        ...
        self.gcn1 = conv.GATConv(in_channels=self.latent_size,
                                 out_channels=int(self.latent_size/self.heads),
                                 batch_norm=True,
                                 heads=self.heads,
                                 concat=True)
        self.gcn2 = conv.GATConv(in_channels=self.latent_size,
                                 out_channels=int(self.latent_size / self.heads),
                                 batch_norm=True,
                                 heads=self.heads,
                                 concat=True)
        self.gcn3 = conv.GATConv(in_channels=self.latent_size,
                                 out_channels=int(self.latent_size / self.heads),
                                 batch_norm=True,
                                 heads=self.heads,
                                 concat=True)
    ...
    def forward(self, gnn_in, centers, agents_per_sample):
        ...
        gnn_out = F.gelu(self.gcn3(x, edge_index, edge_attr))
