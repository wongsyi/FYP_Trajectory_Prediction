...
class RNN_CGNN_2E_Att_Parallel(pl.LightningModule):
    def __init__(self, args):
        super(RNN_CGNN_2E_Att_Parallel, self).__init__()
        self.args = args

        self.save_hyperparameters()

        self.encoder_rnn = EncoderGRU(self.args)
        self.agent_gnn = AgentGnn(self.args)
        self.multihead_self_attention = MultiheadSelfAttention(self.args)
        self.merging = Merging(self.args)
    ...
    def forward(self, batch):
        ...
        out_encoder_rnn = self.encoder_rnn(displ_cat, agents_per_sample)
        out_agent_gnn = self.agent_gnn(
            out_encoder_rnn, centers_cat, agents_per_sample)


        out_self_attention = self.multihead_self_attention(
            out_encoder_rnn, agents_per_sample)


        # Concat from GNN, Att
        out_concat = torch.cat((out_agent_gnn,out_self_attention), dim=1)

        # Vehicle index
        main_vehicle_index = []
        vehicle_offset = 0
        for i in agents_per_sample:
            main_vehicle_index.append(vehicle_offset)
            vehicle_offset += i

        # Remove other vehicles
        out_concat_filtered = out_concat[main_vehicle_index]

        # Half the features
        out_concat_filtered_merged = self.merging(out_concat_filtered)

        ### Decoder ###
        out_linear = self.decoder_residual(out_concat_filtered_merged, self.is_frozen)
...
class EncoderGRU(nn.Module):
    ...
    def forward(self, rnn_in, agents_per_sample):
        # rnn_in are all agents over all samples in the current batch
        # Format for GRU has to be has to be (batch_size, timeseries_length, latent_size), because batch_first=True

        # Initialize the hidden state.
        # rnn_in.shape[0] corresponds to the number of all agents in the current batch
        rnn_hidden_state = torch.randn(
            self.num_layers, rnn_in.shape[0], self.hidden_size, device=rnn_in.device)

        rnn_out, rnn_hidden = self.rnn(rnn_in, rnn_hidden_state)

        # rnn_out is the hidden state over all time steps from the last GRU layer
...
class AgentGnn(nn.Module):
    ...
    def forward(self, gnn_in, centers, agents_per_sample):
        ...
        x = F.gelu(self.gcn1(x, edge_index, edge_attr))
        gnn_out = F.gelu(self.gcn2(x, edge_index, edge_attr))
...
class MultiheadSelfAttention(nn.Module):
    ...
    def forward(self, att_in, agents_per_sample):
        ...
        if len(agents_per_sample) > 1:
            ...
            padded_att_in_swapped = torch.swapaxes(padded_att_in, 0, 1)  # equals to transpose
            ...
            att_out_batch = [x[0:agents_per_sample[i]] # Regroup into number of vehicles per batch
                             for i, x in enumerate(padded_att_in_reswapped)]

            att_out_concat = torch.concat(att_out_batch)
        else:
...
class Merging(nn.Module):
    def __init__(self, args):
        ...
        self.args = args
        self.latent_size = args.latent_size
        self.merging_layer = nn.Linear(in_features=self.latent_size*2,
                                       out_features=self.latent_size
                                       )

    def forward(self, merging_in):
        merging_out = self.merging_layer(merging_in)
