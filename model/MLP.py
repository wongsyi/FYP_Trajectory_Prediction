...
class MLP(pl.LightningModule):
    def __init__(self, args):
        ...
        self.encoder_mlp = EncoderMLP(self.args)
    ...
    def forward(self, batch):
        ...
        # Extract only x,y displacements from (x,y,availability)
        vehicle_xy = displ_cat[:, :, :2]
        batch_size = len(displ)
        vehicle_xy_flattened = torch.reshape(vehicle_xy,(batch_size, -1))

        # Encoder Layer
        ##################################################################################
        out_encoder_mlp = self.encoder_mlp(vehicle_xy_flattened, agents_per_sample)

        # Decoder Layer
        ###################################################################################
        out_linear = self.decoder_residual(out_encoder_mlp, self.is_frozen)
...
class EncoderMLP(nn.Module):
    def __init__(self, args):
        ...
        self.mlp_encoder = nn.Sequential(nn.Linear(in_features=38,
                                         out_features=args.latent_size))
        # Multiple hidden layers
        if self.num_layers > 1:
            for layers in range(self.num_layers-1):
                self.mlp_encoder.append(nn.Linear(in_features=args.latent_size,
                                                  out_features=args.latent_size))


    def forward(self, displacement_in, agents_per_sample):

        mlp_out = self.mlp_encoder(displacement_in)
        return mlp_out
